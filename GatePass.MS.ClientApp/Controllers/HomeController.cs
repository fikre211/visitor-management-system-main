                ...
                if (_signInManager.IsSignedIn(User))
                {
                    ...
                    int requests = await _context.RequestInformation
                        .CountAsync(r => r.CompanyId == _current.Value.Id &&
                                            ...
                                            r.Status == "Pending" &&
                                            !r.Deleted);
                    // make counts company specific
                    int allRegisteredUser = await _userManager.Users.CountAsync(u => u.CompanyId == _current.Value.Id);
                    int employeesCount = 0;
                    if (employeeRole != null)
                    {
                        employeesCount = await _context.UserRoles
                            .Where(ur => ur.RoleId == employeeRole.Id)
                            .Join(_context.Users, ur => ur.UserId, u => u.Id, (ur, u) => u)
                            .CountAsync(u => u.CompanyId == _current.Value.Id);
                    }
                    ...
                    // 1. Data for User Roles Doughnut Chart (company specific)
                    ...
                    int gatekeepersCount = 0;

                    if (adminRole != null)
                    {
                        adminsCount = await _context.UserRoles
                    ...
                    {
                        supervisorsCount = await _context.UserRoles
                            .Where(ur => ur.RoleId == supervisorRole.Id)
                            .Join(_context.Users, ur => ur.UserId, u => u.Id, (ur, u) => u)
                            .CountAsync(u => u.CompanyId == _current.Value.Id);
                    }

                    if (gatekeeperRole != null)
                    {
                        gatekeepersCount = await _context.UserRoles
                            .Where(ur => ur.RoleId == gatekeeperRole.Id)
                            .Join(_context.Users, ur => ur.UserId, u => u.Id, (ur, u) => u)
                            .CountAsync(u => u.CompanyId == _current.Value.Id);
                    }
